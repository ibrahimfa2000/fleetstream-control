...
serve(async (req) => {
  ...
  try {
    ...
    switch (action) {
      case 'getFlowInfo':
        endpoint = '/StandardApiAction_getFlowInfo.action';
        if (params.simNumber) queryParams.append('simNumber', params.simNumber);
        break;

      case 'saveFlowConfig':
        endpoint = '/StandardApiAction_saveFlowConfig.action';
        if (params.simNumber) queryParams.append('simNumber', params.simNumber);
        if (params.flowLimit) queryParams.append('flowLimit', params.flowLimit);
        if (params.warningThreshold) queryParams.append('warningThreshold', params.warningThreshold);
        break;
